...
exports.applyOrder = async (req, res, next) => {
  try {
    ...
    if (!doc) {
      return next(
        new AppError(401, "fail", "No order found with that id"),
        req,
        res,
        next
      );
    ...
    const deliveryman =
      doc.deliveryMan == "" ? null : await User.findById(doc.deliveryMan);
    ...
    switch (req.body.status) {
      case "1":
        fcmController.sendMessage(
          owner.deviceToken,
          "Order Assigned",
          `Your Order (${doc.category}) has been assigned`
        );
        break;
      case "2":
        await sms.sendSMS(
        ...
        fcmController.sendMessage(
          owner.deviceToken,
          "Order In-Progress",
          `Order (${doc.category}) Pick-Up confirmed`
        );
        break;
      case "3":
        if (doc.verifyCode != req.body.otp) {
          return next(
            new AppError(401, "fail", "Incorrect Code!"),
            req,
            res,
            next
          );
        ...
        await User.updateOne(
          { _id: deliveryman._id },
          { $inc: { pendingBalance: Number(doc.amount) } }
        );
        ...
        fcmController.sendMessage(
          owner.deviceToken,
          "Order Completed",
          `Your Order (${doc.category}) has been delivered successfully`
        );
        fcmController.sendMessage(
          deliveryman.deviceToken,
          "Order Completed",
          `You've delivered Order (${doc.category}) successfully`
        );
        break;
      case "4":
        fcmController.sendMessage(
          owner.deviceToken,
          "Order Deleted",
          `Your Order (${doc.category}) has been deleted`
        );
        break;
      case "5":
        fcmController.sendMessage(
          deliveryman.deviceToken,
          "Order Assigned",
          `Kindly confirm Order (${doc.category}) Pick-Up`
        );
        break;
      default:
    }
...
exports.order = async (req, res, next) => {
  try {
    ...
    fcmController.sendMessage(
      owner.deviceToken,
      "Order Created",
      `Your Order (${req.body.category}) has been created`
    );
    ...
    if (closestUsers.length > 0) {
      for (let users of closestUsers) {
        if (
          users.deviceToken != owner.deviceToken &&
          users.deviceToken !== ""
        ) {
          await fcmController.sendMessage(
            users.deviceToken,
            "New Order Alert",
            `A new Order is near you. Open the App to pick it up.`
          );
        }
      }
    }
